#!/bin/bash
if [ $# -lt 2 ]; then
	echo "Usage: $0 imgfilename sizeMiB [product=ar6mx]"
	exit -1 ;
fi
...
if [ -d $mountpoint ]; then
   sudo cp -ravf ../../../out/target/product/$product/*.dtb $mountpoint/
   sudo cp -ravf ../../../out/target/product/$product/uImage $mountpoint/
   sudo cp -ravf ../../../out/target/product/$product/uramdisk.img $mountpoint/
   sudo cp -ravf ../../../out/target/product/$product/boot.scr $mountpoint/
fi
sudo umount $loopdev
sudo losetup -d $loopdev

setuploop $outfilename 2 $RECOVERSTART $RECOVEREND   
sudo cp -rfv ../../../out/target/product/$product/boot/boot.scr /media/RECOVER/
...
if [ -d $mountpoint ]; then
   sudo cp -ravf ../../../out/target/product/$product/*.dtb $mountpoint/
   sudo cp -ravf ../../../out/target/product/$product/uImage $mountpoint/
   sudo cp -ravf ../../../out/target/product/$product/boot.src $mountpoint/
...
sudo mkfs.ext4 -L SYSTEM $loopdev
udisks --mount $loopdev
mountpoint=`mount | grep $loopdev | awk '{ print $3 }'`;
if [ -d $mountpoint ]; then
   sudo cp -ravf ../../../out/target/product/$product/system/* $mountpoint/
fi
sudo umount $loopdev
sudo losetup -d $loopdev
